...
// original: https://raw.githubusercontent.com/spencermountain/wtf_wikipedia/master/plugins/wikis/wiktionary/templates.js
// and https://github.com/spencermountain/wtf_wikipedia/blob/master/plugins/wikis/wiktionary/plugin.js

//wiktionary... who knows. we should atleast try.
const wtf_wiktionary = {};
(function() {
    const templates = {
        etyl: 1,
        mention: 1,
    ...
    conjugations.forEach((name) => {
        templates[name + ' of'] = (tmpl, list, parse) => {
            let obj = parse(tmpl, ['lemma']) // Doesn't always work. for example, {{plural of|fr|cadeau}}
            obj.tags = obj.list
            delete obj.list // (WHY delete!???)
            obj.type = 'form-of'
            list.push(obj)
            return obj.lemma || ''
        }
    ...
    wtf_wiktionary.templates = templates;
    wtf.extend((_models, _templates) => { Object.assign(_templates, templates) });
})();